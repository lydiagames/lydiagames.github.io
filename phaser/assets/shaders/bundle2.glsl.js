...
type: fragment
author: triggerHLM (https://www.shadertoy.com/view/MdXGDH)
uniform.diameter: { "type": "1f", "value": 100.0 }
---
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	vec2 p = vec2(2.0*fragCoord.x-resolution.x, 2.0*fragCoord.y-resolution.y)/diameter;
	...
	uv = (2.0 * uv) - 1.0;
	float beamWidth = (0.7+0.01*cos(uv.x*10.0*tau*0.15*clamp(floor(5.0 + 10.0*cos(time)), 0.0, 10.0))) * abs(1.0 / (30.0 * uv.y));
